    ...
    def get_recommendations(self, user_id: int, num_recommendations: int = 10) -> List[Dict]:
        ...
        try:
            ...
            if user_id not in self.user_encoder.classes_:
                ...
                logger.warning(
                    f"⚠️ User {user_id} not in training data. Available: {available_users[0]}-{available_users[-1]}"
                )
                return self._get_fallback_recommendations(num_recommendations)
            ...
            user_tensor = torch.tensor(
                [encoded_user_id] * len(available_movie_ids), dtype=torch.long, device=self.device
            )
            ...
            for idx in top_indices:
                ...
                recommendations.append(
                    {"movieId": int(original_movie_id), "title": title, "score": score, "genres": genres}
                )
    ...
    def _get_fallback_recommendations(self, num_recommendations: int) -> List[Dict]:
        ...
        try:
            if self.movies_df is not None and not self.movies_df.empty:
                ...
                return [
                    {"movieId": i, "title": f"Popular Movie {i}", "score": 0.5, "genres": "Drama"}
